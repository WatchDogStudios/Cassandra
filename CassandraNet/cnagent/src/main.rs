...
async fn main() -> Result<()> {
    init_tracing();
    info!(event = "agent.start", config = ?config(), "Starting CassandraNet Agent prototype");
    ...
    let tenant_id = std::env::var("CASS_AGENT_TENANT_ID").unwrap_or_default();
    let project_id = std::env::var("CASS_AGENT_PROJECT_ID").unwrap_or_default();
    ...
    loop {
        tokio::select! {
            _ = tokio::signal::ctrl_c() => {
                info!(event = "agent.shutdown", "Shutting down agent");
                break;
            }
                ...
                match client.heartbeat(hb).await {
                    Ok(r) => {
                        ...
                        if r.rotate_credentials {
                            info!(event = "rotate.creds", "Server requested credential rotation");
                        }
                    }
